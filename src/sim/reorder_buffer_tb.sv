...
module reorder_buffer_tb;
    ...
    function automatic void load_program(input bit [31:0] prog [], input string asms []);
        ...
        foreach (prog[k]) begin
            ...
            decoded[k].fu = (d.ControlData.Exception || d.ControlData.Fence)
                            ? -1 : fu_of(d.FunctionalUnitType, k);
        end
    endfunction
    ...
    // -------------------------------------------------------------------------
    typedef enum {EV_COMMIT, EV_STORE, EV_TRAP} ev_kind_e;
    ...
    assign fence_at_head = dispatch_en && !squashed && pc_idx < decoded.size()
                           && decoded[pc_idx].fence;
    assign fence_wait    = fence_at_head && !o_empty;
    assign fence_release = fence_at_head && o_empty;

    int fence_stall_cycles = 0;

    bit want_issue;
    assign want_issue = dispatch_en && !squashed && pc_idx < decoded.size()
                        && !decoded[pc_idx].fence;

    //driven combinationally so that an issue is only ever asserted when the ROB can
    //take it: that keeps shadow_wptr exactly in step with the ROB's own wptr
    assign i_issue_valid     = want_issue && o_ready;
    assign i_issue_pc        = want_issue ? decoded[pc_idx].pc        : '0;
    assign i_issue_rd        = want_issue ? decoded[pc_idx].rd        : '0;
    assign i_issue_regwrite  = want_issue ? decoded[pc_idx].regwrite  : '0;
    assign i_issue_store     = want_issue ? decoded[pc_idx].store     : '0;
    assign i_issue_exception = want_issue ? decoded[pc_idx].exception : '0;
    assign i_issue_exc_cause = want_issue ? decoded[pc_idx].cause     : ILLEGAL_INSTR;
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            for (int f = 0; f < NUM_FU; f++)
                for (int s = 0; s < MAX_LAT; s++)
                    fu_pipe[f][s] <= '{default:'0, cause: ILLEGAL_INSTR};
            pc_idx             <= 0;
            squashed           <= 1'b0;
            fence_stall_cycles <= 0;
        end
        //the flush contract: when the ROB squashes, the units must be squashed too,
        //or a late writeback would mark a reallocated entry complete
            ...
            for (int f = 0; f < NUM_FU; f++) begin
                for (int s = MAX_LAT-1; s > 0; s--)
                    fu_pipe[f][s] <= fu_pipe[f][s-1];
            ...
            //retire the fence locally once everything older has committed
    ...
    function automatic void check_event(input ev_kind_e kind, input string detail);
        ...
        if (e.kind !== kind) begin
            $display("[FAIL] out of order: got %s, expected %s for '%s'",
                     kind.name(), e.kind.name(), e.asm);
            fail_cnt++;
            return;
        end
        case (kind)
            EV_COMMIT:
                if (o_commit_rd !== e.rd || o_commit_value !== e.value) begin
                    $display("[FAIL] commit '%s': got rd=x%0d val=0x%08h, exp rd=x%0d val=0x%08h",
                             e.asm, o_commit_rd, o_commit_value, e.rd, e.value);
                    fail_cnt++;
                end
                else pass_cnt++;
            EV_STORE:
                if (o_store_robtag !== e.tag) begin
                    $display("[FAIL] store '%s': got tag=%0d, exp tag=%0d",
                             e.asm, o_store_robtag, e.tag);
                    fail_cnt++;
                end
                else pass_cnt++;
            EV_TRAP:
                if (o_exception_cause !== e.cause || o_exception_pc !== e.pc) begin
                    $display("[FAIL] trap '%s': got cause=%s pc=0x%08h, exp cause=%s pc=0x%08h",
                             e.asm, o_exception_cause.name(), o_exception_pc,
                             e.cause.name(), e.pc);
                    fail_cnt++;
                end
                else pass_cnt++;
    ...
    always @(posedge clk) begin
        if (!rst) begin
            ...
            if (o_store_valid)
                check_event(EV_STORE, $sformatf("tag=%0d", o_store_robtag));
            if (o_exception_valid)
                check_event(EV_TRAP, $sformatf("cause=%s", o_exception_cause.name()));
    ...
    task automatic expect_program();
        ...
        foreach (decoded[k]) begin
            ...
            if (decoded[k].exception) begin
                e.kind = EV_TRAP; e.cause = decoded[k].cause; e.pc = decoded[k].pc;
                exp_q.push_back(e);
            ...
            else if (decoded[k].regwrite) begin
                e.kind = EV_COMMIT; e.rd = decoded[k].rd; e.value = decoded[k].value;
                exp_q.push_back(e);
            end
            tag++;
    ...
    task automatic run_program(input string name);
        ...
        if (t >= TIMEOUT) begin
            $display("[FAIL] %s: timed out (pc_idx=%0d/%0d empty=%0b outstanding=%0d)",
                     name, pc_idx, decoded.size(), o_empty, exp_q.size());
            fail_cnt++;
        end
        else if (exp_q.size() != 0) begin
            $display("[FAIL] %s: %0d expected event(s) never happened, next is '%s'",
                     name, exp_q.size(), exp_q[0].asm);
            fail_cnt++;
        end
        else begin
    ...
    initial begin
        ...
        asms = '{"lw x1,0(x10)", "add x2,x3,x4", "or x5,x6,x7", "sw x2,4(x10)",
                 "addi x8,x0,1", "beq x1,x2,+12", "slt x9,x1,x2", "sll x11,x1,x2",
                 "lw x12,8(x10)", "xor x13,x1,x2", "sub x14,x1,x2"};
        reset_dut(); load_program(prog, asms); expect_program();
        ...
        load_program(prog, asms); expect_program();
