...
module register_bank_tb;
    ...
    task automatic cycle(input logic [ADDR_WIDTH-1:0]   rs1     = '0,
                         input logic [ADDR_WIDTH-1:0]   rs2     = '0,
                         input logic                    rd_we   = 1'b0,
                         input logic [ADDR_WIDTH-1:0]   rd      = '0,
                         input logic [ROBTAG_WIDTH-1:0] rd_tag  = '0,
                         input logic                    cm_en   = 1'b0,
                         input logic [ADDR_WIDTH-1:0]   cm_addr = '0,
                         input DATA_T                   cm_data = '0,
                         input logic [ROBTAG_WIDTH-1:0] cm_tag  = '0,
                         input logic                    ce_in   = 1'b1);
        @(negedge clk);
        ce            = ce_in;
        i_rs1_addr    = rs1;
        i_rs2_addr    = rs2;
    ...
    initial begin
        ...
        cycle(.rd_we(1), .rd(5'd5), .rd_tag(5'd7),
              .cm_en(1), .cm_addr(5'd5), .cm_data(32'h1111_2222), .cm_tag(5'd3));
        cycle(.rs1(5'd5));
        ...
        cycle(.ce_in(0), .rd_we(1), .rd(5'd13), .rd_tag(5'd9),
              .cm_en(1), .cm_addr(5'd12), .cm_data(32'hFFFF_0000), .cm_tag('0));
