...
import decode_package::ROB_DEPTH;
...
module rams_dist #(
  ...
  always_ff @(posedge clk) begin
    if (rst) begin
      o_spo <= '0;
      o_dpo <= i_di;
      for (int i = 0; i < NUM_REGS; i++) ram[i] <= '0;
    end else if (ce) begin
      if (i_we) begin  //write_first behaviour
        ram[i_a] <= i_di;
        o_spo <= i_di;
      end else o_spo <= ram[i_a];
      o_dpo <= ram[i_dpra];
    end
  end
...
module register_bank_2r1w #(
  ...
  always_ff @(posedge clk) begin
    if (rst) begin
      ...
      r_rs1_rename <= '0;
      r_rs2_rename <= '0;
      for (int i = 0; i < NUM_REGS; i++) renamed_regs[i] <= '0;
    end else if (ce) begin
      ...
      if (i_commit_en && i_rd_we && i_commit_addr == i_rd_addr) begin
        ...
        if (i_rd_we) busy_list[i_rd_addr] <= 1'b1;
      end
