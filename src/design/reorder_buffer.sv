...
module reorder_buffer #(
  ...
  always_ff @(posedge clk) begin
    ...
    end  //everything younger than the trap is squashed, so drop any writeback or
    ...
    //entry ready before its own result exists
      ...
      if (i_jump_valid) rob_status[i_jump_robtag].ready <= 1'b1;
  ...
  always_comb begin
    for (int i = 0; i < NUM_RPORTS; i++) o_fu_op_valid[i] = rob_status[i_fu_op_robtag[i]].ready;
  end
  ...
  always_ff @(posedge clk) begin
    if (rst) begin
      ...
      if (w_trap_flush) begin
        ...
        r_full <= '0;
        o_store_flush <= '1;
      end  //rewind to the slot *after* the jump: only younger instructions are on the
        ...
        if ((i_jump_robtag + 1'b1) != wptr) r_full <= '0;
        o_store_flush <= '1;
      end else if (fifo_write & !r_full) begin
        fifo[wptr] <= tail_data;
        wptr <= wptr + 1'b1;
        if (w_almost_full) r_full <= '1;
      end else if (r_full && fifo_read) r_full <= '0;
    end
  end
  ...
  initial begin
    if (DEBUG)
      $monitor(
          "[%0t] [ROB] rob_issue=%0b rob_input=0x%0h rob_commit=%0b rob_head=0x%0h empty=%0b full=%0b",
          $time,
          fifo_write,
          tail_data,
          fifo_read,
  ...
  always_ff @(posedge clk) begin
    if (rst) begin
      ...
      r_head_robtag <= '0;
      r_empty <= '1;
    end  //rptr has already moved past both stages, so everything still in the head
    ...
    //presented head also stops the trap from re-firing on the next cycle
      ...
      if (fifo_read & !r_empty) begin
        rptr <= rptr + 1'b1;
        if (w_almost_empty) r_empty <= '1;
      end else if (r_empty && fifo_write) r_empty <= '0;
    end
  end
  ...
  // Stitch the NUM_RPORTS operand-fetch read ports together with the single
  // commit read port. Portable SV cannot concatenate an unpacked array with a
  // scalar directly in a port map -- plain {} is unpacked-array concatenation
  // that xsim rejects, and '{} is an assignment pattern that will not expand the
  // array -- so map the elements explicitly. Ordering matches the previous
  // {i_fu_op_robtag, rptr} concat: indices 0..NUM_RPORTS-1 are the FU ports and
  // index NUM_RPORTS is the commit port.
  logic  [TAG_WIDTH-1:0] w_raddr [NUM_RPORTS+1];
  DATA_T                 w_rdata [NUM_RPORTS+1];

  always_comb begin
    for (int i = 0; i < NUM_RPORTS; i++) begin
      w_raddr[i]       = i_fu_op_robtag[i];
      o_fu_op_value[i] = w_rdata[i];
    end
    w_raddr[NUM_RPORTS] = rptr;               // commit read address
    o_commit_value      = w_rdata[NUM_RPORTS]; // committed result value
  end
